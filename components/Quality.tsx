                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {allQualityEntries.map(entry => (
                            <tr key={entry.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-xs truncate" title={entry.notes}>{entry.notes}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {allQualityEntries.length === 0 && (
                    <p className="text-center text-gray-500 py-8">Nenhum lançamento de qualidade foi feito neste turno ainda.</p>
                )}
            </div>
        </div>
    );
