...
const Products: React.FC = () => {
  const { products, openModal, deleteProduct } = useAppContext();

  const handleEdit = (product: Product) => {
    openModal('ADD_EDIT_PRODUCT', { product });
  };
  ...
  return (
    <div className="p-4 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => openModal('ADD_EDIT_PRODUCT')} className="bg-brand-green text-white font-semibold py-2 px-4 rounded-lg hover:bg-brand-light-green transition-colors">
          Adicionar Produto
        </button>
      </div>
