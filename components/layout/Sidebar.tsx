...
const GaugeIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m12 14 4-4"/><path d="M3.34 19a10 10 0 1 1 17.32 0"/><path d="M16 12h.01"/></svg>
);
...
const ParetoIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M8 18v-4"/><path d="M12 18v-8"/><path d="M16 18v-1"/><path d="M20 18v-6"/><path d="M3 3v18h18"/><path d="m3 9 5.5-5.5"/><path d="M8.5 3.5 16 11l4 1"/></svg>
);
...
const ChevronsLeftIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m11 17-5-5 5-5"/><path d="m18 17-5-5 5-5"/></svg>
);
const ChevronsRightIcon: React.FC<React.SVGProps<SVGSVGElement>> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="m6 17 5-5-5-5"/><path d="m13 17 5-5-5-5"/></svg>
);
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView }) => {
  const { openModal, activeShift, exportActiveShiftToExcel, settings, installPrompt, triggerInstallPrompt, currentUser, logout } = useAppContext();
  ...
  return (
    <div className={`flex flex-col bg-brand-green text-white transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        <nav className="mt-4">
            ...
            <DropdownMenu title="Registros" icon={<HistoryIcon className="h-5 w-5"/>} isOpen={openMenu === 'registros'} onClick={() => toggleMenu('registros')} isCollapsed={isSidebarCollapsed}>
                ...
                {isAdmin && <NavItem icon={<EditIcon className="h-5 w-5" />} label="Editar Produção" isActive={currentView === AppView.EDIT_DATA} onClick={() => setCurrentView(AppView.EDIT_DATA)} disabled={!activeShift} isCollapsed={isSidebarCollapsed} />}
                {isAdmin && <NavItem icon={<ZapIcon className="h-5 w-5" />} label="Editar ITH" isActive={currentView === AppView.EDIT_ITH_DATA} onClick={() => setCurrentView(AppView.EDIT_ITH_DATA)} disabled={!activeShift} isCollapsed={isSidebarCollapsed} />}
            </DropdownMenu>
            
            {isAdmin && <DropdownMenu title="Cadastros" icon={<PackageIcon className="h-5 w-5"/>} isOpen={openMenu === 'cadastros'} onClick={() => toggleMenu('cadastros')} isCollapsed={isSidebarCollapsed}>
            ...
            <DropdownMenu title="Sistema" icon={<SettingsIcon className="h-5 w-5"/>} isOpen={openMenu === 'sistema'} onClick={() => toggleMenu('sistema')} isCollapsed={isSidebarCollapsed}>
                {isAdmin && <NavItem icon={<SettingsIcon className="h-5 w-5" />} label="Configurações" isActive={currentView === AppView.SETTINGS} onClick={() => setCurrentView(AppView.SETTINGS)} isCollapsed={isSidebarCollapsed} />}
                {isAdmin && <NavItem icon={<UsersIcon className="h-5 w-5" />} label="Usuários" isActive={currentView === AppView.USERS} onClick={() => setCurrentView(AppView.USERS)} isCollapsed={isSidebarCollapsed} />}
                {installPrompt && (
                    <NavItem 
      ...
      <div className="p-4 border-t border-brand-light-green space-y-2">
        {!isSidebarCollapsed && currentUser && (
             <div className="text-center p-2 bg-black bg-opacity-20 rounded-lg">
                ...
                <p className="text-xs text-gray-300 capitalize">{currentUser.role === 'admin' ? 'Administrador' : 'Visualizador'}</p>
             </div>
        )}
        <div className="flex items-center justify-between gap-2">
          <button 
              onClick={logout} 
              className={`flex-1 flex items-center p-2 rounded-lg hover:bg-red-500 text-white transition-colors ${isSidebarCollapsed ? 'justify-center' : ''}`}
              title="Sair"
          >
              <LogOutIcon className="w-5 h-5" />
              <span className={`ml-2 whitespace-nowrap ${isSidebarCollapsed ? 'hidden' : 'inline'}`}>Sair</span>
          </button>
          <button 
            onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} 
            className="flex items-center justify-center p-2 rounded-lg hover:bg-brand-light-green text-white transition-colors"
